...
export default class renderRow extends Component {
  ...
  render() {
    ...
    return (
      <div key={key} style={selected?{backgroundColor:"#bae7ff"}:null} onClick={()=>handleClick(key)} className={styles.renderRow}>
        <div className={styles.renderCol}>
          {date}
        </div>
        <div className={styles.renderName}>
          <b><Link to={`/master/${ledger.cust_id}`}>{ name }</Link></b>
        </div>
        <div className={styles.renderCol}>
          { ledger.comments }
        </div>
