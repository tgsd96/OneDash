...
export default class LedgerView extends Component {
  ...
  selectLedgerForEditing = id => {
    ...
    if (!showDialog) this.toggleDialog();
  };
  ...
  render() {
    ...
    return (
      <Card bordered={false}>
        <Col span={24}>
          <Button shape="round" type="ghost" onClick={this.createNewLedger}>
                  ...
                  {ledgerToMap.all.map(id => {
                    ...
                    return (
                      <RenderRow
                        ...
                        ledger={cledger}
                        handleClick={this.selectLedgerForEditing}
                        name={masters[cledger.cust_id] && masters[cledger.cust_id].name}
                      />
                    );
                  })}
                </div>
